...
class AdapterTest {
    ...
    fun `urls json adapts to strings`() {
        assertEquals(listOf("url1", "url2"), "[\"url1\",\"url2\"]".model)
        assertEquals(emptyList<String>(), "[]".model)
    }

    @Test
    fun `malformed urls json throws exception`() {
        assertThrows(JsonSyntaxException::class.java) { "[url1, url2".model }
    }

    @Test
    fun `strings adapts to urls json`() {
        assertEquals("[\"url1\",\"url2\"]", listOf("url1", "url2").local)
    }
}
